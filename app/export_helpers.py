...
def save_excel_button(label, sheets, ds, filename, key, *, help=None, subfolder=None):
    ...
    **Pass a callable when a sheet is expensive to build.** The iterable form is
    ...
    test sheet is the case that matters — it is a fifth of that page's cost, and
    computing it just to have a button sit unclicked is a fifth of every rerun.

    A workbook rather than a folder of CSVs because the sheets are read together —
    a summary, the per-fly values behind it, and the tests over those — and because
