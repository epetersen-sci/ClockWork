"""HMM model selection — how many states, and which emission model.

Rendered as the first tab of the HMM page. It was a page of its own, immediately
above the one that consumes its answer; two pages for one decision and its
consequence is one more click than the work needs.

The body is unchanged from that page except that its ``st.stop()`` calls are
``return``: st.stop ends the whole script, which inside a tab would take the
...
def render():
    ...
    st.markdown(
        ...
        "Results guide the parameters you set on the **Analysis** tab."
    )
    ...
    st.info(
        ...
        "**Analysis** tab, or the model chosen here will not describe the data "
    ...
    **Comparing emission models:**
    ...
    - Then inspect state parameters on the **Analysis** tab to check biological
